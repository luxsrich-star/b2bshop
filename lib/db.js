...
export function getShopSettings(slug) {
  ...
  if (!read(fp)) write(fp, { name:"Мой магазин", logoText:"ММ", logoImg:null, whatsapp:"", useSharedCatalog:true });
  return read(fp);
}
...
// ─── history log ──────────────────────────────────────────────────────────────
export function getShopLog(slug) {
  const fp = path.join(shopDir(slug), "log.json");
  if (!read(fp)) write(fp, []);
  return read(fp);
}
export function saveShopLog(slug, data) { write(path.join(shopDir(slug), "log.json"), data); }

export function addLogEntry(slug, entry) {
  const log = getShopLog(slug);
  log.unshift({
    id:      Date.now(),
...
export function findProductByName(slug, productName) {
  ...
  const all = settings.useSharedCatalog ? sharedCat.products : ownCat.products;
  const name = productName.toLowerCase().trim();
  return all.find(p => p.name.toLowerCase().includes(name)) || null;
}
