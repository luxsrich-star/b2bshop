...
function AnimPane({role,dir,onDone,children}){
  ...
  return <div ref={ref} style={{position:"absolute",inset:0,overflowY:"auto",background:"var(--bg)",willChange:"transform"}}>{children}</div>;
}
...
const BoxIcon  =()=><svg width="36" height="36" viewBox="0 0 24 24" fill="currentColor" style={{opacity:0.15}}><path d="M20.54 5.23l-1.39-1.68C18.88 3.21 18.47 3 18 3H6c-.47 0-.88.21-1.16.55L3.46 5.23C3.17 5.57 3 6.02 3 6.5V19c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V6.5c0-.48-.17-.93-.46-1.27zM12 17.5L6.5 12H10v-2h4v2h3.5L12 17.5zM5.12 5l.81-1h12l.94 1H5.12z"/></svg>;
...
const PrintIcon=()=><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M19 8H5c-1.66 0-3 1.34-3 3v6h4v4h12v-4h4v-6c0-1.66-1.34-3-3-3zm-3 11H8v-5h8v5zm3-7c-.55 0-1-.45-1-1s.45-1 1-1 1 .45 1 1-.45 1-1 1zm-1-9H6v4h12V3z"/></svg>;
...
// CARD WIDTH — fixed, never changes
const CARD_W = 155;

// ── ProductCard — fixed size always ──────────────────────────────────────────
  ...
  return (
    <div style={{
      ...
      <div style={{height:120, background:"var(--surface2)", display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden", flexShrink:0}}>
        {p.img
          ? <img src={p.img} alt={p.name} loading="lazy" style={{width:"100%",height:"100%",objectFit:"cover"}} onLoad={e=>e.target.style.opacity=1} onError={e=>e.target.style.display="none"}/>
          : <BoxIcon/>}
      </div>
      {/* body */}
      <div style={{padding:"9px 10px 11px", display:"flex", flexDirection:"column", flex:1}}>
        {/* name — fixed 2 lines */}
        <div style={{
          fontSize:12, fontWeight:600, lineHeight:1.35, color:"var(--text)",
          marginBottom:4,
          display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical",
          overflow:"hidden", minHeight:32
        }}>{p.name}</div>
        <div style={{fontSize:13, fontWeight:700, color:"var(--text)", marginBottom:3}}>{p.price} ₽</div>
        <div style={{
        ...
        {!out ? (
          <>
            <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", border:"1.5px solid var(--border)", borderRadius:9, padding:"4px 7px", marginBottom:6, background:"var(--surface2)"}}>
              <button onClick={()=>setQty(Math.max(1,qty-1))} style={{background:"none",border:"none",cursor:"pointer",fontSize:17,color:"var(--text)",lineHeight:1,padding:"0 2px"}}>−</button>
              <span style={{fontSize:12,fontWeight:600,color:"var(--text)"}}>{qty}</span>
              <button onClick={()=>setQty(Math.min(p.stock,qty+1))} style={{background:"none",border:"none",cursor:qty>=p.stock?"not-allowed":"pointer",fontSize:17,color:qty>=p.stock?"var(--text3)":"var(--text)",lineHeight:1,padding:"0 2px"}}>+</button>
            </div>
            <button onClick={onAdd} style={{width:"100%",background:"var(--accent)",color:"var(--accent-t)",border:"none",borderRadius:9,padding:"9px 0",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
              В корзину
            </button>
          </>
        ) : (
          <button disabled style={{width:"100%",background:"var(--surface2)",color:"var(--text3)",border:"1px solid var(--border)",borderRadius:9,padding:"9px 0",fontSize:12,cursor:"not-allowed",fontFamily:"inherit"}}>
            Нет в наличии
          </button>
        )}
      </div>
...
function CatChip({ cat, onClick }) {
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{fontSize:13, fontWeight:600, color:"var(--text)", lineHeight:1.3}}>{cat.name}</div>
    </div>
  );
}

// ── HRow — horizontal scroll row, always fixed-width cards ───────────────────
function HRow({ title, items, renderItem }) {
  return (
    <div style={{marginBottom:24}}>
      {title && <div style={{fontSize:15,fontWeight:700,padding:"0 16px",marginBottom:10,color:"var(--text)"}}>{title}</div>}
      <div style={{
        display:"flex", gap:10,
        ...
        {items.map((item, i) => (
          <div key={item.id||i} style={{flexShrink:0, scrollSnapAlign:"start"}}>
            {renderItem(item)}
          </div>
        ))}
      </div>
...
export default function ShopPage() {
  ...
  // multiprices modal
  const [multiModal,setMultiModal] = useState(null); // product
  ...
  function addToCart(p, priceOverride) {
    const price = priceOverride ?? p.price;
    const qty   = Math.min(quantities[p.id]||1, p.stock);
    if(qty<=0) return;
    setCart(c=>{
      const ex=c.find(i=>i.id===p.id&&i.price===price);
      if(ex) return c.map(i=>i.id===p.id&&i.price===price?{...i,qty:Math.min(i.qty+qty,p.stock)}:i);
      return [...c, {id:p.id,name:p.name,price,qty,stock:p.stock}];
    });
    setQtys(q=>({...q,[p.id]:1}));
  }
  function chQty(id,price,d){
    setCart(c=>c.map(i=>i.id===id&&i.price===price?{...i,qty:Math.max(1,Math.min(i.qty+d,i.stock||999))}:i));
  }
        ...
        {catId!==null&&(
          <>
            {childCats.length>0&&!search&&(
              <HRow title="Подразделы" items={childCats}
                renderItem={cat=><CatChip cat={cat} onClick={()=>{nav.go([...stack,cat.id]);setSearch("");}}/>}/>
            )}
            <HRow title={search?"Результаты":"Товары"} items={filteredCat}
              renderItem={p=>(
                <ProductCard p={p}
                  qty={quantities[p.id]||1}
                  setQty={v=>setQtys(q=>({...q,[p.id]:Math.max(1,Math.min(v,p.stock||1))}))}
  ...
  const inpStyle={border:"1.5px solid var(--border)",borderRadius:12,padding:"10px 13px",fontSize:13,outline:"none",width:"100%",boxSizing:"border-box",fontFamily:"inherit",background:"var(--surface)",color:"var(--text)"};
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)"}}>
      ...
      {/* Header */}
      <div style={{borderBottom:"1px solid var(--border)",padding:"0 12px",display:"flex",alignItems:"center",justifyContent:"space-between",height:56,position:"sticky",top:0,background:"var(--surface)",backdropFilter:"blur(14px)",zIndex:20,boxShadow:"0 2px 10px var(--shadow)",gap:8}}>
        ...
        <div style={{display:"flex",gap:7,flexShrink:0}}>
          ...
          <button onClick={()=>{setCartOpen(true);setStep("cart");}} style={{background:"none",border:"1.5px solid var(--border)",borderRadius:11,padding:"7px 12px",fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",gap:7,color:"var(--text)"}}>
            <CartIcon/>
      ...
      {/* Breadcrumbs */}
      <div style={{background:"var(--surface)",borderBottom:"1px solid var(--border)",padding:"8px 14px",display:"flex",gap:5,fontSize:12,color:"var(--text2)",flexWrap:"wrap",alignItems:"center"}}>
      ...
      {/* Search */}
      <div style={{background:"var(--surface)",borderBottom:"1px solid var(--border)",padding:"8px 14px"}}>
      ...
      {/* Content */}
      <div style={{position:"relative",overflow:"hidden",minHeight:"calc(100vh - 162px)"}}>
      ...
      {/* ── MULTIPRICES MODAL ── */}
      {multiModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:200,display:"flex",alignItems:"flex-end",animation:"fadeIn .2s ease"}}
          onClick={e=>e.target===e.currentTarget&&setMultiModal(null)}>
          <div style={{background:"var(--surface)",width:"100%",borderRadius:"22px 22px 0 0",padding:"20px 20px 40px",boxShadow:"0 -8px 40px var(--shadow)",animation:"slideUp .28s cubic-bezier(0.4,0,0.2,1)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div>
                <div style={{fontSize:15,fontWeight:700,color:"var(--text)"}}>{multiModal.name}</div>
                <div style={{fontSize:12,color:"var(--text2)",marginTop:2}}>Выберите вариант цены</div>
              </div>
              <button onClick={()=>setMultiModal(null)} style={{background:"none",border:"none",color:"var(--text2)",cursor:"pointer",fontSize:22,lineHeight:1}}>✕</button>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {/* default price */}
              <button onClick={()=>{addToCart(multiModal,multiModal.price);setMultiModal(null);}}
                style={{background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:12,padding:"14px 16px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",fontFamily:"inherit"}}>
                <div style={{textAlign:"left"}}>
                  <div style={{fontSize:13,fontWeight:600,color:"var(--text)"}}>Розничная цена</div>
                  <div style={{fontSize:11,color:"var(--text2)",marginTop:2}}>Стандартная цена</div>
                </div>
                <div style={{fontSize:16,fontWeight:700,color:"var(--text)"}}>{multiModal.price} ₽</div>
              </button>
              {/* multi prices */}
              {(multiModal.multiPrices||[]).map((mp,i)=>(
                <button key={i} onClick={()=>{addToCart(multiModal,mp.price);setMultiModal(null);}}
                  style={{background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:12,padding:"14px 16px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",fontFamily:"inherit"}}>
                  <div style={{textAlign:"left"}}>
                    <div style={{fontSize:13,fontWeight:600,color:"var(--text)"}}>{mp.name}</div>
                    {mp.minOrder&&<div style={{fontSize:11,color:"var(--text2)",marginTop:2}}>от {mp.minOrder} ₽</div>}
                  </div>
                  <div style={{fontSize:16,fontWeight:700,color:"var(--text)"}}>{mp.price} ₽</div>
                </button>
              ))}
            </div>
          </div>
      ...
      {cartOpen&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:100,display:"flex",alignItems:"flex-end",animation:"fadeIn .2s ease"}}
          ...
          <div style={{background:"var(--surface)",width:"100%",maxHeight:"88vh",borderRadius:"22px 22px 0 0",display:"flex",flexDirection:"column",boxShadow:"0 -8px 40px var(--shadow)",animation:"slideUp .3s cubic-bezier(0.4,0,0.2,1)"}}>
              ...
              <>
                <div style={{padding:"18px 20px 12px",borderBottom:"1px solid var(--border)",flexShrink:0}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <button onClick={()=>setCartOpen(false)} style={{background:"none",border:"none",color:"var(--text2)",cursor:"pointer",fontSize:22}}>✕</button>
                  </div>
                </div>
                {cart.length===0
                        ...
                        <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"11px 0",borderBottom:"1px solid var(--border)"}}>
                          ...
                          <div style={{display:"flex",alignItems:"center",gap:8}}>
                            <div style={{display:"flex",alignItems:"center",gap:6,border:"1.5px solid var(--border)",borderRadius:10,padding:"4px 8px",background:"var(--surface2)"}}>
                              <button onClick={()=>chQty(item.id,item.price,-1)} style={{background:"none",border:"none",cursor:"pointer",fontSize:16,color:"var(--text)"}}>−</button>
                              <span style={{fontSize:13,minWidth:18,textAlign:"center",fontWeight:600,color:"var(--text)"}}>{item.qty}</span>
                              <button onClick={()=>chQty(item.id,item.price,1)} style={{background:"none",border:"none",cursor:item.qty>=(item.stock||999)?"not-allowed":"pointer",fontSize:16,color:item.qty>=(item.stock||999)?"var(--text3)":"var(--text)"}}>+</button>
                            </div>
                            <div style={{fontSize:13,fontWeight:600,minWidth:60,textAlign:"right",color:"var(--text)"}}>{item.qty*item.price} ₽</div>
                            <button onClick={()=>setCart(c=>c.filter((_,j)=>j!==i))} style={{background:"none",border:"none",color:"var(--text3)",cursor:"pointer",display:"flex"}}><TrashIcon/></button>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {step==="form"&&(
              <>
                <div style={{padding:"16px 20px 12px",borderBottom:"1px solid var(--border)",flexShrink:0}}>
                  <button onClick={()=>setStep("cart")} style={{background:"none",border:"none",color:"var(--text2)",cursor:"pointer",fontSize:12,padding:0,marginBottom:4,fontFamily:"inherit"}}>‹ Назад</button>
                ...
                <div style={{overflowY:"auto",flex:1,padding:"16px 20px"}}>
                  <div style={{display:"flex",flexDirection:"column",gap:12}}>
                    ...
                    <div>
                      ...
                      <input value={form.tg} onChange={e=>setForm(f=>({...f,tg:e.target.value.replace(/^@/,"")}))} placeholder="username" style={inpStyle}/>
                    </div>
                    <div>
                    ...
                    <label style={{display:"flex",gap:10,alignItems:"flex-start",cursor:"pointer"}}>
                      <input type="checkbox" checked={form.agreed} onChange={e=>setForm(f=>({...f,agreed:e.target.checked}))} style={{width:16,height:16,accentColor:"var(--accent)",flexShrink:0,marginTop:2}}/>
            ...
            {step==="done"&&curOrder&&(
              <>
                <div style={{padding:"18px 20px 12px",borderBottom:"1px solid var(--border)",flexShrink:0,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <button onClick={()=>{setCartOpen(false);setStep("cart");}} style={{background:"none",border:"none",color:"var(--text2)",cursor:"pointer",fontSize:22}}>✕</button>
                </div>
                <div style={{overflowY:"auto",flex:1,padding:"20px"}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                    <button onClick={()=>{setReceiptOrd(curOrder);setCartOpen(false);}} style={{width:"100%",background:"var(--accent)",color:"var(--accent-t)",border:"none",borderRadius:13,padding:"13px 0",fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:7,fontFamily:"inherit"}}>
